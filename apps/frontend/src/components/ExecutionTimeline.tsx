...
export const ExecutionTimeline: React.FC<ExecutionTimelineProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-80 max-w-full bg-bg-surface border-l border-border-subtle shadow-xl z-50 flex flex-col h-full transform transition-transform duration-300">
      {/* Drawer Header */}
      <div className="p-4 border-b border-border-subtle flex items-center justify-between bg-bg-muted">
        <div>
          ...
          <span className="text-3xs text-text-secondary font-mono truncate max-w-[200px] block">
            ID: {job?.jobId || "N/A"}
          </span>
        </div>
        <button
      ...
      {/* Overview Block */}
      {job && (
        <div className="p-4 bg-bg-page border-b border-border-subtle space-y-2 text-2xs">
          <div className="flex justify-between">
            <span className="text-text-secondary">Language:</span>
            <span className="font-mono font-bold text-text-primary">{job.language || "javascript"}</span>
          </div>
          <div className="flex justify-between">
      ...
      {/* Timeline List */}
      <div className="p-4 flex-grow overflow-y-auto space-y-4 font-sans select-none">
        {events.length === 0 ? (
